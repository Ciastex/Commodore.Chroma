...
namespace Commodore.EVIL.Execution
{
    public partial class Interpreter
    {
        ...
        {
            int setMemoryValue(int addr, int value, MemoryGetNode.OperandSize size)
            {
                if (size == MemoryGetNode.OperandSize.Byte)
                {
                    if (value > byte.MaxValue)
                        throw new RuntimeException("Operand size mismatch, expected at most a byte.", memorySetNode.Line);
                ...
                else if (size == MemoryGetNode.OperandSize.Word)
                {
                    if (value > short.MaxValue)
                        throw new RuntimeException("Operand size mismatch, expected at most a word.", memorySetNode.Line);
                ...
                else if (size == MemoryGetNode.OperandSize.Dword)
                {
                    if (value > int.MaxValue)
                        throw new RuntimeException("Operand size mismatch, expected at most a dword.", memorySetNode.Line);
                ...
                switch (memorySetNode.MemoryCellNode.Size)
                {
                    ...
                    case MemoryGetNode.OperandSize.Dword:
                        ...
                        if (value + 1 > int.MaxValue)
                            Memory.Poke((int)address, int.MinValue);
                        else
                            Memory.Poke((int)address, (int)(value + 1));
            ...
            else throw new RuntimeException("Internal error: unexpected memory set operation type.", memorySetNode.Line);
        }
    }
}
